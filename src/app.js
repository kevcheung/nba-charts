...
app.post("/testPost", (req, res) => { // .save() - SAVE ONE DOCUMENT TO MONGODB COLLECTION
...
app.get("/testGet/:name", (req, res) => { //POSTMAN DOES NOT NEED COLON FOR REQUESTS AND MUST BE CASE SENSITIVE
  ...
  console.log("API HIT!!!!!", params.name);
  TestModel.find({ name: params.name })
    .then((test) => {
      //NEEDED TO PUT 'PARAMS.NAME' HERE INSTEAD OF JUST 'PARAMS'
      console.log("NAME PROCESSED!!!!", test);
...
app.put("/testPatch/:name", (req, res) => {
  //TAKES IN NAME AND REQUESTS WILL OVERWRITE WHAT IS CURRENTLY IN DB
  const { params } = req;
  // res.setHeader('Access-Control-Allow-Origin', '*');
  TestModel.updateOne({ name: params.name }, req.body)
    .then((replace) => {
...
app.delete("/testDelete/:name", (req, res) => {
  const { params } = req;
  TestModel.deleteOne({ name: params.name }, req.body)
    .then((del) => {
